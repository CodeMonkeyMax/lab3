...
void XImage_From_XWD_File(XImage *pxim, FILE *fp) {
  ...
  int waste = 0;
  waste = fread(pxim->data, numbytestoread, 1, fp);
...
static unsigned char bmp_header[54] = {
    0x42, 0x4D, 0x00, 0x00, 0x00, 0x00, // size of bmp file-overwrite
    0x00, 0x00, 0x00, 0x00, 0x36, 0x00, 0x00, 0x00, 0x28,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // width of bitmap in
                                              // pixels-overwrite
    0x00, 0x00, 0x00, 0x00, // height of bitmap in pixles-overwrite
    0x01, 0x00, 0x18, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, // size of raw bmp data after this bmp_header
    0x13, 0x0B, 0x00, 0x00, 0x13, 0x0B, 0x00, 0x00, 0x00,
...
static int get_int_from_little_endian(int p) {
  ...
  r = ((d << 24) & 0xff000000) | ((c << 16) & 0x00ff0000) |
...
// return 0 if failure, else return 1
{
  ...
  if (bmp_header[0] != 0x42)
    return 0;
  if (bmp_header[1] != 0x4D)
    return 0;
